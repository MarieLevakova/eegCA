...
#
#
# SparseCointegration_Lasso<-function(p,Y,X,Z,r,alpha.init=NULL,beta.init=NULL,max.iter=10,conv=10^-2,
#                                     lambda.gamma=matrix(seq(from=0.01,to=0.001,length=5),nrow=1),
#                                     rho.glasso=seq(from=1,to=0.1,length=5),
#                                     lambda_beta=matrix(seq(from=0.1,to=0.001,length=100),nrow=1),
...
#   RESID <- Y -Z%*%beta.init%*%t(alpha.init)
...
#   {
#     # Obtain Gamma
#     FIT1<-NTS_GAMMALassoReg(Y=Y,X=X,Z=Z,Pi=Pi.init,p=p,Omega=Omega.init,
#                             lambda.gamma=lambda.gamma,glmnetthresh=glmnetthresh)
#     # Obtain Alpha
#     FIT2<-NTS.ALPHA.Procrusted(Y=Y,X=X,Z=Z,ZBETA=FIT1$ZBETA,r=r,Omega=Omega.init,P=FIT1$P,beta=beta.init)
#     # Obtain Beta and Omega
#     FIT3<-NTS_BETA(Y=Y,X=X,Z=Z,ZBETA=FIT1$ZBETA,r=r,Omega=Omega.init,P=FIT1$P,alpha=FIT2$ALPHA,alphastar=FIT2$ALPHAstar,lambda_beta=lambda_beta,rho.glasso=rho.glasso,cutoff=cutoff,glmnetthresh=glmnetthresh)
#
#
#     # Check convergence
#     beta.new<-FIT3$BETA
...
#   }
#
#   out<-list(BETAhat = FIT3$BETA, ALPHAhat = FIT2$ALPHA/dt, it = it,
#             ZBETA = FIT1$ZBETA/dt, OMEGA = FIT3$OMEGA/dt)
# }
#
# NTS_GAMMALassoReg<-function(Y,X,Z,Pi,Omega,lambda.gamma=matrix(seq(from=1,to=0.01,length=10),nrow=1),p,cutoff=0.8,intercept=F,glmnetthresh=1e-04){
...
# }
#
# NTS_BETA<-function(Y,X,Z,ZBETA,r,Omega,P,alpha,alphastar,lambda_beta=matrix(seq(from=2,to=0.001,length=100),nrow=1),rho.glasso,cutoff,intercept=F,glmnetthresh=1e-04){
